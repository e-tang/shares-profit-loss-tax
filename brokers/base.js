    ...
    calculate_profit(holding, transaction, financial_year) {
        ...
        while(last_transaction) {
            ...
            if (transaction.total) {
                // if we have the total information it means we can include not just 
                // the price but also the brokerage+GST
                let transaction_avg_price = transaction.total / transaction.quantity;
                current_total = transaction_avg_price * profit_quantity;
            }
            else {
                // so we won't be able to include the brokerage+GST to reflect the real cost
