function Transaction() {
    this.id = 0;            // sequential id for the transaction
    this.uuid = "";         // unique id for the transaction
    this.date = new Date();
    ...
    // FP Markets cTrader closed-position fields
    this.is_closed_position = false;
    this.date_open = null;
    ...
    // Binance closed-pair fields
    this.is_binance_closed_pair = false;
    this.cost = 0;
    this.proceeds = 0;
    this.source_count = 0;
    this.quote_cost = 0;
    this.quote_proceeds = 0;
    this.quote_profit = 0;
...
function Portfolio() {
    ...
    this.history_years = new Set();         // year -> Holding[], for each financial year
}
...
function Profit() {
    ...
    this.profit = 0;                        // overall profit
    this.gross_profit = 0;                  // broker-reported P/L before commission and swaps
    this.commission = 0;
    this.swaps = 0;

    this.discount_eligible = false;
                                            // discount_eligible = true if the holding period is more than 12 months
    this.transaction_open;                 // list of transactions for the round-trip trade
    this.transaction_close;                // list of transactions for the round-trip trade

    this.trade_type;                        // trade type: buy, sell, dividend, bonus, split, merge, etc.
    this.total_trades = 0;                  // total number of trades / transactions since the holding is first bought
}
...
function Holding() {
    ...
    this.records = [];                      // list of transactions, adjusted transactions for building up current holding after all closed trades are removed
    this.last_cos = null;                   // the date of the holding having consolidation / split
}
...
function Trade() {
    ...
    this.toString = function() {
        if (this.symbol == undefined)
            return "";
        return this.symbol + " (" + this.quantity + ", " + this.type + "): open@" + this.cost_price + ", close@" + this.close_price + ", profit: " + this.profit.toFixed(2);
    }
}
